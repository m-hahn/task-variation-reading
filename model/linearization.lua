require 'nn.LinearizationChooser'
assert(false)
...
-- takes a syntax tree and stochastically outputs a surface string
function buildLinearizationReaderModule()
  local parent                = nn.Identity()()
  local firstChild                = nn.Identity()()
  local secondChild                = nn.Identity()()
  local thirdChild                = nn.Identity()()
  local fourthChild                = nn.Identity()()

  local prev_c           = nn.Identity()()
  local prev_h           = nn.Identity()()

  local linearization = nn.LinearizationChooser()(parent, firstChild, secondChild, thirdChild, fourthChild)

  local chosenNextNode = nn.SelectTable(1)(linearization)
  local probability = nn.SelectTable(2)(linearization)

  local next_c, next_h = lstm.lstm(chosenNextNode, prev_c, prev_h)
  local module = nn.gModule({prev_c, prev_h, parent, firstChild, secondChild, thirdChild, fourthChild},{next_c, next_h, probability, chosenNextNode})
...
function buildLinearizationReconstructorModule()
   return create_network(true)
end

function setupLinearization()
...
function fpLinearization(corpus, startIndex, endIndex)
...
function bpLinearization(corpus, startIndex, endIndex)
  ...
  local targetTensors = buildInputTensors(corpus, startIndex, endIndex)
   ...
   -- do it for actor network
     ...
     local reward = torch.add(nll,params.TOTAL_ATTENTIONS_WEIGHT,totalAttentions)
     ...
     rewardBaseline = 0.8 * rewardBaseline + 0.2 * torch.sum(reward) * 1/params.batch_size
      ...
      for i = maxLength, 1, -1 do
          ...
          -- compute reinforcement contribution

         local whatToMultiplyToTheFinalDerivative = torch.CudaTensor(params.batch_size)
         local linearizationEntropyFactor =  torch.CudaTensor(params.batch_size)
         for j=1,params.batch_size do
            linearizationEntropyFactor[j] =  params.ENTROPY_PENALTY * (math.log(probabilities[i][j][1]) - math.log(1 - probabilities[i][j][1])) -- TODO
            if attention_decisions[i][j] == 0 then
                whatToMultiplyToTheFinalDerivative[j] = -1 / (1 - probabilities[i][j][1])
            else
                whatToMultiplyToTheFinalDerivative[j] = 1 / (probabilities[i][j][1])
            end
         end
         ...
         factorsForTheDerivatives:add(attentionEntropyFactor)
          ...
          model.dsR[3] = factorsForTheDerivatives -- probabilities
          model.dsR[4]:zero() -- the choice of the child
...
function printStuffForLinearization(perp, actor_output, since_beginning, epoch, numberOfWords)
...
function saveLinearization(numberOfWords)
