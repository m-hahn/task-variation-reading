...
function neatQA.buildNeuralGraph()
  ...
  local reader = nn.RecursorMod(autoencoding.create_network(neatQA.ALSO_DO_LANGUAGE_MODELING, true, true), nil, {reader_c[0], reader_h[0]}, 1, 3, {true,true,false})
  reader = nn.Sequencer(reader)

  local inputWords = nn.Identity()()
  local reader2Rec = nn.RecursorMod(autoencoding.create_network(neatQA.ALSO_DO_LANGUAGE_MODELING, true, true):cuda(), nil, {reader_c[0], reader_h[0]}, 1, 3, {true,true,false}):cuda()
  reader2 = nn.Sequencer(reader2Rec)(inputWords)
  local lastStates = nn.SelectTable(-1)(reader2)
  local lastStates1 = nn.SelectTable(1)(lastStates)
  local lastStates2 = nn.SelectTable(2)(lastStates)
--  local withActor = neatQA.createSimpleAnswerNetwork()({lastStates1, lastStates2})
  ...
  local withActor = (nn.LogSoftMax():cuda())(inActor)
...
--  neatQA.answerCriterion = nn.ClassNLLCriterion()
  ...
  rewardBaseline = 0
   ...
   twoLayerLSTMGraph = buildDeepLSTMReader() 
     ...
     tempDoNotUse, neatQA.fullModel = neatQA.buildNeuralGraph()
...
--    print(tmp[1])
  --  print(tmp[2])
   ...
   --else
      ...
      for i = 1,neatQA.maximalLengthOccurringInInput[1] do
        model.dsFull[1][i] = {}
        model.dsFull[1][i][1] = torch.CudaTensor(params.batch_size,params.rnn_size):zero()
        model.dsFull[1][i][2] = torch.CudaTensor(params.batch_size,params.rnn_size):zero()
        model.dsFull[1][i][3] = torch.CudaTensor(params.batch_size,params.vocab_size):zero()
      end
...
function neatQA.printStuff(perp, actor_output, since_beginning, epoch, numberOfWords)
            ...
            for l = 1, params.batch_size do
               ...
               if  (true or math.random() < 0.001) then
                  ...
                  for j=2,params.seq_length do
                    ...
                    local predictedScoresLM, predictedTokensLM = torch.min( neatQA.fullOutput[1][j-1][3][l],1)
                    ...
                    io.write("  "..math.exp(-neatQA.fullOutput[1][j-1][3][l][neatQA.inputTensors[j][l]]).."\n")
                  end
...
--               print(readChunks.corpus[l].text)
               print("ANSW "..answerID)
               print("PROB "..neatQA.fullOutput[1][l][3][answerID])
               local negSample = math.random(math.min(10, neatQA.fullOutput[1][l][3]:size()[1]))
               print("NEGATIVE EX PROB "..neatQA.fullOutput[1][l][3][negSample].." ("..negSample..")")
               if (math.abs(neatQA.fullOutput[1][l][3][answerID]) <= math.abs(neatQA.fullOutput[1][l][3][negSample])) then
                 correct = correct + 1.0
               else
