...
function qaAttentionAnswerer.createAnswerNetworkWithMemoryAndQuestionAttention()
...
--  prev_c_join = nn.PrintLayer("prev_c_join",1,true)(prev_c_join)
  ...
  local shortcut = nn.SelectTable(5)(prev_c_table)
...
--  attention_score = nn.PrintLayer("attention_score6b")(attention_score)
  ...
  attention_score = nn.MulConstant(0.0)(attention_score)
...
--  prev_c = nn.PrintLayer("prev_c10")(prev_c)
  ...
  local includingShortcut = nn.JoinTable(2)({prev_c,shortcut,question2})

--  local includingShortcut = nn.Linear(5*params.rnn_size,30)(includingShortcut)

--  local includingShortcut = nn.ReLU()(includingShortcut)
  local decisionLinear = nn.Linear(5*params.rnn_size,NUMBER_OF_ANSWER_OPTIONS)(includingShortcut)
...
function qaAttentionAnswerer.createAnswerNetworkWithMemoryAndQuestionAttentionNORMAL()
  local prev_c_table = nn.Identity()()
  local backward_prev_c_table = nn.Identity()()
