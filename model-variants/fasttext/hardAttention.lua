...
function hardAttention.makeAttentionDecisions(i, inputTensor, surprisalValue, attInputTensor)
   --inputTensor = perturbInputTensor(inputTensor)
   if attInputTensor == nil then
      attInputTensor = inputTensor
   end
--                 print(attInputTensor)
  --               print(1639)
      ...
      if use_attention_network then 
           if params.TASK == 'autoencoding' then
              ...
              if params.INCLUDE_NUMERICAL_VALUES then
                 local surprisalValue = torch.CudaTensor(params.batch_size,1)
                 for l=1,params.batch_size do
                    surprisalValue[l][1] = numericalValues.numericalValuesImporter.values[l][i]
                 end
                 attention_scores[i], baseline_scores[i] = unpack(attentionNetworks[i]:forward({attInputTensor, reader_c[i-1], surprisalValue}))
              else
                 attention_scores[i] , baseline_scores[i] = unpack(attentionNetworks[i]:forward({attInputTensor, reader_c[i-1]}))
              end
              if surprisalValue ~= nil then
                 crash()
              end
           ...
           elseif params.TASK == 'neat-qa' then
              assert(false)
              attention_scores[i] = attentionNetworks[i]:forward({attInputTensor, reader_c[i-1]})
              if surprisalValue ~= nil then
                 crash()
              end
           else
